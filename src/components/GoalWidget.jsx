...
const GOALS = [
  { key: 'weight_target', labelKey: 'gw.label_weight_target', icon: 'scale', unitKey: 'gw.unit_kg', color: '#02d1ba' },
  { key: 'sessions_week', labelKey: 'gw.label_sessions_week', icon: 'dumbbell', unitKey: 'gw.unit_sessions', color: '#a78bfa' },
  { key: 'steps_day', labelKey: 'gw.label_steps_day', icon: 'shoe', unitKey: 'gw.unit_steps', color: '#fb923c' },
];
      ...
      {GOALS.map(g => {
        ...
        return (
          <div key={g.key} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 12, marginBottom: 8 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 10, color: '#6b7280', fontWeight: 700 }}>{label.toUpperCase()}</div>
              {isEdit ? (
                <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                  ...
                  <button aria-label={t('gw.aria_validate')} onClick={() => save(g.key)} style={{ background: g.color, color: '#0d0d0d', border: 'none', borderRadius: 8, padding: '6px 14px', fontWeight: 900, cursor: 'pointer', minHeight: 36, minWidth: 44, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                    <AppIcon name="check" size={16} color="#0d0d0d" strokeWidth={2.5} />
                  </button>
                </div>
              ) : <div style={{ fontSize: 20, fontWeight: 900, color: goal ? '#f5f5f5' : '#374151' }}>{goal ? Number(goal.target_value).toLocaleString(intlLocale()) + ' ' + unit : '—'}</div>}
            </div>
            {!isEdit && (
              <button onClick={() => { haptic.light(); setEditing(g.key); setVal(goal?.target_value||''); }} aria-label={goal ? t('gw.aria_edit') : t('gw.aria_define')} style={{ background: g.color+'20', border: '1px solid '+g.color+'40', borderRadius: 10, padding: '8px 12px', color: g.color, fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5, minHeight: 36 }}>
                {goal ? <AppIcon name="edit" size={12} color={g.color} /> : <><AppIcon name="plus" size={12} color={g.color} />{t('gw.btn_define')}</>}
              </button>
            )}
          </div>
        );
