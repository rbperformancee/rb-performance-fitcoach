                ...
                {prog ? (
                  <div style={{ background: "#1a1a1a", borderRadius: 10, padding: "12px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button onClick={async () => { if(window.confirm("Supprimer ce programme ?")) { await supabase.from("programmes").update({is_active:false}).eq("id",prog.id); toast("Programme supprime."); onClose(); } }} style={{ fontSize: 10, fontWeight: 700, color: "#ef4444", background: "none", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 7, padding: "5px 10px", cursor: "pointer" }}>Supprimer</button>
                    </div>
                  </div>
                ) : (
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
          <div style={{background:"#111",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:28,maxWidth:320,width:"100%",textAlign:"center"}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={() => setConfirmDeleteProg(null)} style={{flex:1,padding:13,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,color:"rgba(255,255,255,0.5)",fontSize:14,fontWeight:600,cursor:"pointer"}}>Annuler</button>
              <button onClick={async () => { await supabase.from("programmes").update({is_active:false}).eq("id",confirmDeleteProg); setConfirmDeleteProg(null); toast("Programme supprimé."); onClose(); }} style={{flex:1,padding:13,background:"#ef4444",border:"none",borderRadius:12,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer"}}>Supprimer</button>
            </div>
          </div>
        </div>
      )}
